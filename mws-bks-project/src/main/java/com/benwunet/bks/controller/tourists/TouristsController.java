...
public class TouristsController {
    ...
    public ResponseResult getCampus() {
        ...
        List<BksCampus> list = campusService.list(new QueryWrapper<BksCampus>().select("campus_id", "campus_name").orderByAsc("index_by"));
    ...
    public ResponseResult getMajorSchoolList(String majorId, Integer page) {
        ...
        if(ObjectUtils.isEmpty(majorSchoolList)){
            dataMap.put("major", null);
            dataMap.put("schools", null);
        }else {
  ...
  */
    ...
    public ResponseResult matchUniversity(@RequestBody ExamPredictionQueryVO queryVo) {
        IPage<BksSchoolVO> pages = bksSchoolService.matchUniversity(
                queryVo.getProvinceId(), queryVo.getScore(), new Page<>(queryVo.getPageCurrent(), queryVo.getPageSize()));
        //计算匹配度
        for (BksSchoolVO schoolVO : pages.getRecords()) {

            schoolVO.setMatchDegree("极高");
        }
    ...
    public ResponseResult matchMajor(@RequestBody ExamPredictionQueryVO queryVo) {
        List<BksUniversityEnrollGdf> enrollGdfList = universityEnrollGdfService.matchMajor(queryVo.getProvinceId(), queryVo.getScore());
