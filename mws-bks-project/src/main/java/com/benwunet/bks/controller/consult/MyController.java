...
import com.benwunet.bks.model.vo.MyConcernVO;
...
public class MyController extends BaseController {
    ...
    @PostMapping("/myConcernSchool/{userId}")
    @ApiOperation(value = "备考生小程序我关注的高校", notes = "备考生小程序我关注的高校")
    public ResponseResult myConcernSchool(@RequestBody MyConcernVO myConcernVO) {
        QueryWrapper<BksUserSchoolMajor> bksUserSchoolMajorQueryWrapper = new QueryWrapper<>();
        bksUserSchoolMajorQueryWrapper.eq("user_id", myConcernVO.getUserId());
        bksUserSchoolMajorQueryWrapper.isNotNull("school_id");
        int count = majorService.count(bksUserSchoolMajorQueryWrapper);
        if(!ObjectUtils.isEmpty(myConcernVO.getPageCurrent()) && !ObjectUtils.isEmpty(myConcernVO.getPageSize())){
            bksUserSchoolMajorQueryWrapper.last(" limit " + (myConcernVO.getPageCurrent() - 1) * myConcernVO.getPageSize()
                    + "," + myConcernVO.getPageSize());
        }
        List<BksUserSchoolMajor> BksUserSchoolMajorList = majorService.list(bksUserSchoolMajorQueryWrapper);
        List<BksSchool> bksSchoolList = new ArrayList<>();
        BksUserSchoolMajorList.forEach(x -> {
            ...
            bksSchoolList.add(bksSchool);
        });
        return success(count, bksSchoolList);
    }

    @PostMapping("/myConcernMajor/{userId}")
    @ApiOperation(value = "备考生小程序我关注的专业", notes = "备考生我关注的专业")
    public ResponseResult myConcernMajor(@RequestBody MyConcernVO myConcernVO) {
        QueryWrapper<BksUserSchoolMajor> bksUserSchoolMajorQueryWrapper = new QueryWrapper<>();
        bksUserSchoolMajorQueryWrapper.eq("user_id", myConcernVO.getUserId());
        bksUserSchoolMajorQueryWrapper.isNotNull("major_id");
        int count = majorService.count(bksUserSchoolMajorQueryWrapper);
        if(!ObjectUtils.isEmpty(myConcernVO.getPageCurrent()) && !ObjectUtils.isEmpty(myConcernVO.getPageSize())){
            bksUserSchoolMajorQueryWrapper.last(" limit " + (myConcernVO.getPageCurrent() - 1) * myConcernVO.getPageSize() + ","
                    + myConcernVO.getPageSize());
        ...
        List<BksUserSchoolMajor> BksUserSchoolMajorList = majorService.list(bksUserSchoolMajorQueryWrapper);
        List<BksMajor> bksMajorList = new ArrayList<>();
        BksUserSchoolMajorList.forEach(x -> {
            QueryWrapper<BksMajor> bksSchoolQueryWrapper = new QueryWrapper<>();
            bksSchoolQueryWrapper.eq("major_id", x.getMajorId());
            BksMajor bksMajor = bksMajorService.getOne(bksSchoolQueryWrapper);
            bksMajorList.add(bksMajor);
        });
        return success(count, bksMajorList);
    }

    @PostMapping("/cancelMyConcernMajor/{userId}")
    @ApiOperation(value = "取消收藏", notes = "取消收藏")
    public ResponseResult cancelMyConcernMajor(@RequestBody MyConcernVO myConcernVO) {
        QueryWrapper<BksUserSchoolMajor> bksUserSchoolMajorQueryWrapper = new QueryWrapper<>();
        bksUserSchoolMajorQueryWrapper.eq("user_id", myConcernVO.getUserId());
        if (!ObjectUtils.isEmpty(myConcernVO.getSchoolId())) {
            bksUserSchoolMajorQueryWrapper.eq("school_id", myConcernVO.getSchoolId());
        }
        if (!ObjectUtils.isEmpty(myConcernVO.getMajorId())) {
            bksUserSchoolMajorQueryWrapper.eq("major_id", myConcernVO.getMajorId());
        }
        if (!ObjectUtils.isEmpty(myConcernVO.getProfessorId())) {
            bksUserSchoolMajorQueryWrapper.eq("professor_id", myConcernVO.getProfessorId());
        }
        majorService.remove(bksUserSchoolMajorQueryWrapper);
        return success();
    }

    @PostMapping("/concern/{userId}")
    @ApiOperation(value = "收藏", notes = "收藏")
    public ResponseResult concern(@RequestBody BksUserSchoolMajor bksUserSchoolMajor) {
        majorService.save(bksUserSchoolMajor);
        return success();
    }
