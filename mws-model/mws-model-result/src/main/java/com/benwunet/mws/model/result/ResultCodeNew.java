...
public enum ResultCodeNew {
    ...
    /**构造方法*/
    ResultCodeNew(int code, String message)
    {
        this.code = code;
     ...
     * 通过code返回枚举
     */
    public static ResultCodeNew parse(int code){
        ResultCodeNew[] values = values();
        for (ResultCodeNew error : values) {
    ...
    public static void main(String[] args) {
        ResultCodeNew errorCode = Arrays.stream(ResultCodeNew.values()).filter(x -> x.getCode() == 1014031111).findAny().orElse(ResultCodeNew.EXECUTING_REQUEST_ERROR);
