...
//    }
    ...
    @ApiOperation(value = "小程序一键登录", notes = "小程序一键登录")
    @PostMapping("/sys/appletLogin")
    public ResponseResult getKeyUserInfo(@RequestBody WechatAppletLoginDTO dto) {
        ...
        if(ObjectUtils.isEmpty(byWechatApplet)){
            return ResponseResult.app(1, 0, "请求成功，请绑定openid！（根据openid查询用户为空）", null);
        }
        Map<String, String> parameters = new HashMap<>();
        parameters.put(OAuth2Utils.GRANT_TYPE, "password");
        parameters.put(OAuth2Utils.CLIENT_ID, SystemClientInfo.CLIENT_ID);
        ...
        parameters.put(OAuth2Utils.SCOPE, SystemClientInfo.CLIENT_SCOPE);
        /**为了支持多类型登录，这里在username后拼装上登录类型，同时为了服务端校验，我们也拼上tempCode*/
        parameters.put("username", byWechatApplet.getMobile() + "|" + CredentialType.WECHAT_OPENID.name());
        /**微信登录无需密码，但security底层有密码校验，我们这里将手机号作为密码，认证中心采用同样规则即可*/
        parameters.put("password", byWechatApplet.getMobile());
        Map<String, Object> tokenInfo = oauth2Client.postAccessToken(parameters);
        saveLoginLog(infoDTO.getOpenId(), "微信登陆");
        Map<String, Object> data = new HashMap<>();
        data.put("access_token", tokenInfo.get("access_token"));
        data.put("user",byWechatApplet);
        return ResponseResult.app(0, 0, "登录成功！", data);
    }

    @ApiOperation(value = "短信登录", notes = "短信登录")
    @PostMapping("/sys/textLogin")
    public ResponseResult textLogin(String mobile, String key, String code) {
        Map<String, String> parameters = new HashMap<>();
        parameters.put(OAuth2Utils.GRANT_TYPE, "password");
        parameters.put(OAuth2Utils.CLIENT_ID, SystemClientInfo.CLIENT_ID);
        parameters.put("client_secret", SystemClientInfo.CLIENT_SECRET);
        ...
        Map<String, Object> data = new HashMap<>();
        data.put("access_token", tokenInfo.get("access_token"));
        data.put("user",sysUserClient.getLoginPubSysUser(mobile));
        saveLoginLog(mobile, "手机号短信登陆");
        return ResponseResult.app(0, 0, "登录成功！", data);
    }
